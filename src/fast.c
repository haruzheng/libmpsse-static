...
static int fast_build_block_buffer(struct mpsse_context *mpsse, uint8_t cmd,
				   ...
				   int *buf_size)
{
	...
	*buf_size = 0;
	...
	if ((cmd == mpsse->tx || cmd == mpsse->txrx) &&
	    (i + size) <= sizeof(fast_rw_buf) && data != NULL) {
	...
	if (is_valid_context(mpsse)) {
		if (mpsse->mode) {
			while (n < size) {
				...
				if (fast_build_block_buffer(
					    mpsse, mpsse->tx,
					    (unsigned char *)(data + n), txsize,
					    &buf_size) == MPSSE_OK) {
					if (raw_write(mpsse, fast_rw_buf,
						      buf_size) == MPSSE_OK) {
						n += txsize;
					} else {
						return MPSSE_FAIL;
 ...
 * Returns MPSSE_OK on success, MPSSE_FAIL on failure.
 */
int FastTransfer(struct mpsse_context *mpsse, const char *wdata, char *rdata,
		 size_t size)
{
	...
	if (is_valid_context(mpsse)) {
		if (mpsse->mode >= SPI0 && mpsse->mode <= SPI3) {
			while (n < size) {
				...
				if (fast_build_block_buffer(
					    mpsse, mpsse->txrx,
					    (unsigned char *)(wdata + n),
					    rxsize, &data_size) == MPSSE_OK) {
					if (raw_write(mpsse, fast_rw_buf,
						      data_size) == MPSSE_OK) {
						n += raw_read(
							mpsse,

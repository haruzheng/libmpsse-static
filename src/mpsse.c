...
struct vid_pid supported_devices[] = {
	{ 0x0403, 0x6010,
	  "FT2232 Future Technology Devices International, Ltd" },
	{ 0x0403, 0x6011,
	  "FT4232 Future Technology Devices International, Ltd" },
	{ 0x0403, 0x6014,
...
struct mpsse_context *MPSSE(enum modes mode, int freq, int endianess)
{
	...
	for (i = 0; supported_devices[i].vid != 0; i++) {
		if ((mpsse = Open(supported_devices[i].vid,
				  supported_devices[i].pid, mode, freq,
				  endianess, IFACE_A, NULL, NULL)) != NULL) {
			if (mpsse->open) {
				mpsse->description =
...
static struct mpsse_context *
OpenIndexInternal(int vid, int pid, enum modes mode, int freq, int endianess,
		  int interface, const char *description, const char *serial,
		  int index, unsigned usbBus, unsigned usbAddress);

struct mpsse_context *OpenUsbDev(enum modes mode, unsigned bus,
				 unsigned address)
{
	return OpenIndexInternal(0, 0, mode, 0, 0, IFACE_A, NULL, NULL, 0, bus,
				 address);
}
...
{
	return OpenIndex(vid, pid, mode, freq, endianess, interface,
...
struct mpsse_context *OpenIndex(int vid, int pid, enum modes mode, int freq,
				...
				int index)
{
	return OpenIndexInternal(vid, pid, mode, freq, endianess, interface,
				 description, serial, index, 0, 0);
}
	...
	if (mpsse) {
		...
		if (ftdi_init(&mpsse->ftdi) == 0) {
...
#if HAVE_LIBFTDI1_5
				res = ftdi_usb_open_bus_addr(
					&mpsse->ftdi, usbBus, usbAddress);
#endif
			} else {
				res = ftdi_usb_open_desc_index(&mpsse->ftdi,
							       vid, pid,
							       description,
							       serial, index);
			}
			...
			if (res == 0) {
				...
				status |= ftdi_set_latency_timer(&mpsse->ftdi,
								 LATENCY_MS);
				status |= ftdi_write_data_set_chunksize(
					&mpsse->ftdi, CHUNK_SIZE);
				status |= ftdi_read_data_set_chunksize(
					&mpsse->ftdi, CHUNK_SIZE);
				status |= ftdi_set_bitmode(&mpsse->ftdi, 0,
							   BITMODE_RESET);
				...
				if (status == 0) {
					...
					if (mpsse->mode != BITBANG) {
						ftdi_set_bitmode(&mpsse->ftdi,
								 0,
								 BITMODE_MPSSE);

						if (SetClock(mpsse, freq) ==
						    MPSSE_OK) {
							if (SetMode(mpsse,
								    endianess) ==
							    MPSSE_OK) {
...
int SetMode(struct mpsse_context *mpsse, int endianess)
{
	...
	if (mpsse) {
		...
		case I2C:
			...
			setup_commands[setup_commands_size++] =
				ENABLE_3_PHASE_CLOCK;
			break;
		case GPIO:
			break;
		default:
		...
		if (retval == MPSSE_OK && setup_commands_size > 0) {
			retval = raw_write(mpsse, setup_commands,
					   setup_commands_size);
		}
	...
	if (is_valid_context(mpsse)) {
		if (mpsse->mode) {
			while (n < size) {
				...
				buf = build_block_buffer(
					mpsse, mpsse->tx,
					(const unsigned char *)data + n, txsize,
					&buf_size);
				if (buf) {
					retval =
						raw_write(mpsse, buf, buf_size);
					n += txsize;
					free(buf);
		...
		if (mpsse->mode) {
			...
			if (buf) {
				...
				while (n < size) {
					...
					data = build_block_buffer(mpsse,
								  mpsse->rx,
								  sbuf, rxsize,
								  &data_size);
					if (data) {
						retval = raw_write(mpsse, data,
								   data_size);
						free(data);
 ...
 */
#ifdef SWIGPYTHON
swig_string_data Transfer(struct mpsse_context *mpsse, const char *data,
			  size_t size)
#else
		...
		if (mpsse->mode >= SPI0 && mpsse->mode <= SPI3) {
			...
			if (buf) {
				...
				while (n < size) {
					...
					txdata = build_block_buffer(
						mpsse, mpsse->txrx,
						(unsigned char *)(data + n),
						rxsize, &data_size);
					if (txdata) {
						retval =
							raw_write(mpsse, txdata,
								  data_size);
						free(txdata);
...
int Stop(struct mpsse_context *mpsse)
{
	...
	if (is_valid_context(mpsse)) {
		...
		if (mpsse->mode == I2C) {
			retval |=
				set_bits_low(mpsse, (mpsse->pidle & ~DO & ~SK));
		}
...
int SetDirection(struct mpsse_context *mpsse, uint8_t direction)
{
	...
	if (is_valid_context(mpsse)) {
		if (mpsse->mode == BITBANG) {
			if (ftdi_set_bitmode(&mpsse->ftdi, direction,
					     BITMODE_BITBANG) == 0) {
				retval = MPSSE_OK;
			}
		}
